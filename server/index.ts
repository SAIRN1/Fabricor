...
import {
  ...
  salesEntries, priceBookItems, edgeDetails, materialDetails,
  costSettings, industryBenchmarks, claudeConversations,
...
app.get("/api/reports", requireAuth, async (req, res) => {
  try {
    const userId = (req.session as any).userId;
    const reports = await db.select().from(weeklyReports).where(eq(weeklyReports.userId, userId)).orderBy(desc(weeklyReports.year), desc(weeklyReports.weekNumber)).limit(52);
    res.json(reports);
...
app.post("/api/reports", requireAuth, async (req, res) => {
  try {
    const userId = (req.session as any).userId;
    const [report] = await db.insert(weeklyReports).values({ ...req.body, userId }).returning();
    res.json(report);
